...
size_t WasmFunction::compile(const webassembly_t::func_t *func, const std::unique_ptr<webassembly_t::functype_t> &funcType,
  ...
  if (func->expr().size() > 0) {
    ...
    assembler::assembleExpression(it, expr.end(), variables, registerPool, controlStack, wasmStack, trapHandler, functionCallPatches, loadAddressPatches,
                                  type_section, function_section, functionTable, machinecode);
