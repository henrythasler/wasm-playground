...
TEST(local1, function2) {
  auto wasmModule = helper::loadModule("local.1.wasm");
  auto machinecode = wasmModule.getWasmFunction("type-param-i64")->getMachinecode();
  auto wasmFunction = tiny::make_wasm_function<void, wasm::wasm_i32_t>(machinecode);
  wasmFunction(2);
}
...
TEST(local1, function4) {
  ...
  auto wasmFunction = tiny::make_wasm_function<void, wasm::wasm_i64_t, wasm::wasm_i32_t, wasm::wasm_i32_t>(machinecode);
  wasmFunction(0, 0, 0);
}
